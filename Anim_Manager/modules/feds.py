...
from Anim_Manager import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from Anim_Manager.modules.helper_funcs.handlers import CMD_STARTERS
from Anim_Manager.modules.helper_funcs.misc import is_module_loaded, send_to_list
from Anim_Manager.modules.helper_funcs.chat_status import is_user_admin
from Anim_Manager.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from Anim_Manager.modules.helper_funcs.string_handling import markdown_parser
from Anim_Manager.modules.disable import DisableAbleCommandHandler

import Anim_Manager.modules.sql.feds_sql as sql
from Anim_Manager.modules.helper_funcs.alternate import send_message
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text("Failed to set federation rules.")
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Anim_Manager_fbanned_users.json"
            ...
            with BytesIO(str.encode(backups)) as output:
                ...
                update.effective_message.reply_document(document=output, filename="Anim_Manager_fbanned_users.csv",
